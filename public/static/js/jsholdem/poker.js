...
function goto_main() {
  var form = document.getElementById("form-game-end");
  var inputElement = document.getElementById("player-input");
  inputElement.value = JSON.stringify(players[0]);
  form.submit();
}
function goto_select() {
  ...
  fetch('/game/nl-holdem-rebet', {
    ...
    .then(data => {
      // Check if the response message starts with "You Need Extra"
      if (data.message.startsWith('You Need Extra')) {
        alert(data.message); // Show an alert with the received message
      } else {
        new_round(); // Call a function named new_round()
      }
    })
    .catch(error => {
      // Handle any errors
...
function my_pseudo_alert(text) {
  var html = "<html><body topmargin=25 bottommargin=0 bgcolor='black' onload='document.f.y.focus();'>" +
    "<p size=+2 style='padding: 20px;padding-top: 25vh;font-size: 30px !important;'>" + text +
    "</p><form name=f><input style='font-size: 100px !important; cursor: pointer; background: black; border-radius: 4px; border: 2px solid; font: inherit; line-height: 1; margin: 0.5em; padding: 1em; color: #ffa260;' name=y type=button value='  OK  ' " +
...
function new_game_continues(req_no_opponents) {
  var my_players = [
    new player("Michael", 0, "", "", "", 0, 0),
    new player("Julia", 0, "", "", "", 0, 0),
    new player("Annette", 0, "", "", "", 0, 0),
    new player("Richard", 0, "", "", "", 0, 0),
    new player("Jennifer", 0, "", "", "", 0, 0),
    new player("Leonardo", 0, "", "", "", 0, 0),
    new player("Mark", 0, "", "", "", 0, 0),
    new player("Butternut", 0, "", "", "", 0, 0),
...
function handle_end_of_round() {
  ...
  var quit_text = "Goto Main";
  // var quit_func = new_game;
  var quit_func = goto_main;
  var continue_text = "Rebet";
  // var continue_func = new_round;
  var continue_func = goto_select;
